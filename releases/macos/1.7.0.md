...
- - Moved the "Clear Up Next" button to the left of the toolbar - Changed the "Enabled Continuous Playback" to a new infinity symbol button - Added a refresh button that returns the Continuous Playback playlist back to its original order when using Append mode, or loads a new randomized set of Episodes when using Shuffle mode. - Added a new 'gear' button to the playlist sidebar that displays a popover for changing Continuous Playback settings
